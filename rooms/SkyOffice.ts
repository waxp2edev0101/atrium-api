...
export class SkyOffice extends Room<OfficeState> {
  ...
  async onCreate(options: IRoomData) {
    ...
    this.onMessage(
      ...
      (
        client,
        message: { x: number; y: number; anim: string; accountId: string; userId: string }
      ) => {
    ...
    this.onMessage(
      ...
      (client, message: { receiver: any; content: string; createdAt: number }) => {
        // update the message array (so that players join later can also see the message)
        // this.dispatcher.dispatch(new ChatMessageUpdateCommand(), {
        //   client,
        ...
        this.state.players.forEach((player, key) => {
          if (player.name === message.receiver.username) {
            this.clients.forEach((cli) => {
              if (cli.sessionId === key) {
                cli.send(Message.DIRECT_CHAT_MESSAGE, {
                  username: username,
                  content: message.content,
    ...
    // when a player send a community chat message, update the message array and broadcast to all connected clients except the sender
    this.onMessage(
      Message.COMMUNITY_CHAT_MESSAGE,
      (
        client,
        message: {
          username: string
        ...
        // broadcast to all currently connected clients except the sender (to render in-game dialog on top of the character)
        this.broadcast(Message.COMMUNITY_CHAT_MESSAGE, message, { except: client })
      }
    )
  }
